...
class TestIncludedLanelets(unittest.TestCase):
    ...
    def test_goalreached(self, development: bool=False):
        ...
        success_scenarios, failed_scenarios = self._analyse_scenarios(scenario_names,
                                                                      development=development,
                                                                      use_cr2023_challenge=False)

        if(len(failed_scenarios) > 0):
            raise ValueError(f"Not all scenarios passed: \n " +
                             f"succeded easy: {len(success_scenarios)} --  {success_scenarios} \n"
                             f"failed easy: {len(failed_scenarios)}  --  {failed_scenarios} \n")

    def _analyse_scenarios(self, scenarios: List[str], use_cr2023_challenge: bool = False,
                           scenarios_path: str = None,
                           development: bool=False) -> Tuple[List[str], List[str]]:
        """
