...
def initialize():
    # load config file
    configs = hf.load_config_file(os.path.join(os.path.dirname(__file__), 'batch_processor_config.yaml'))
    # create a logger
    logger = hf.initialize_logger("Batch processor", configs)
    ...
    existing_scenario_ids = set(hf.get_existing_scenario_ids(scenarios_root_folder))
    ...
    return configs, logger, scenarios_root_folder, scenario_ids
...
def setup_and_run(configs, logger, scenarios_root_folder, scenario_ids):
    ...
    inch_in_cm = 2.54
    figsize = [60, 30]
    existing_scenario_ids_paths_dict = hf.get_existing_scenarios(scenarios_root_folder)

    counter = 1
    for idx, (scenario, planning_problem_set) in enumerate(hf.load_scenarios(scenarios_root_folder, scenario_ids)):
        ...
        try:
            ...
            if plot_and_save_scenarios:
                ...
                plot_limits = hf.get_plot_limits(route_obj.route, scenario, border=15)
    ...
    list_result_sceanrios.append(scenarios_with_different_ids)
    list_result_sceanrios.append(scenarios_with_initial_state_error)
    return num_of_scenarios_to_solve, list_result_sceanrios


def logger_output(logger, num_of_scenarios_to_solve, list_result_sceanrios):
    scenarios_path_found = list_result_sceanrios[0]
    scenarios_path_not_found = list_result_sceanrios[1]
    scenarios_exception = list_result_sceanrios[2]
    scenarios_with_different_ids = list_result_sceanrios[3]
    scenarios_with_initial_state_error = list_result_sceanrios[4]
...
if __name__ == "__main__":
    configs, logger, scenarios_root_folder, scenario_ids = initialize()
    num_of_scenarios_to_solve, list_result_sceanrios = setup_and_run(configs, logger, scenarios_root_folder,
                                                                     scenario_ids)
    logger_output(logger, num_of_scenarios_to_solve, list_result_sceanrios)
