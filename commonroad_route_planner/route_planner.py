...
class RoutePlanner:
    ...
    def extend_reference_path_at_start(cls, reference_path: np.ndarray, initial_position_cart: np.ndarray,
        ...
        """
        Adds additional points at start along a line between first two points of reference path.
    ...
    @classmethod
    def extend_reference_path_at_end(cls, reference_path: np.ndarray, final_position: np.ndarray,
                                       additional_lenght_in_meters: float = 5.0,
                                       distance_threshold_in_meters: float = np.inf) -> Tuple[np.ndarray, bool]:
        """
