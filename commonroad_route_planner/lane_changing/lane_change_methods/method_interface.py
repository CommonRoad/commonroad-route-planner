...
from commonroad_route_planner.lane_changing.lane_change_methods.polynomial_change import (
    generate_cubic_spline_ref_path, generate_quintic_spline_ref_path)
...
class MethodInterface:
    ...
    def compute_lane_change_ref_path(self,
    ...
    def _compute_cubic_spline_lane_change(self,
                                          start_point: np.ndarray,
                                          end_point: np.ndarray,
                                          step_size=0.1
                                         ) -> np.ndarray:
        ...
        self._logger.info(f'Computed cubic lane change')

        return ref_path

    def _compute_quintic_spline_lane_change(self,
                                          start_point: np.ndarray,
                                          end_point: np.ndarray,
                                          step_size=0.1
                                         ) -> np.ndarray:
        ...
        self._logger.info(f'Computed quintic lane change')
