...
class MapMatcher:
    def __init__(self, lanelet_network: LaneletNetwork, relax_consistency_constraint: int = 2):
    ...
    def map_mathing(self, trajectory: List[State]) -> List[int]:
        ...
        m = gp.Model("Map Matching")  # academic gurobi lic https://support.gurobi.com/hc/en-us/articles/360040541251
        ...
        # constraints
        m.addConstr(x[:, :] <= occupancy_matrix[:, :])  # only choose lanelets that are available

        for k in range(number_time_steps):
            m.addConstr(gp.quicksum(x[:, k]) <= 1)  # only match one lanelet at each timestep

        # consider lanelet network topology
        for lt_id in occurring_lanelet_ids:
            preceding_lanelets = self.lanelet_network.find_lanelet_by_id(lt_id).predecessor.copy()
            ...
            for k in range(number_time_steps - 1 - self.relax_consistency_constraint):
                ...
                m.addConstr(gp.quicksum(gp.quicksum(x[preceding_lanelets, k:k + 1 + self.relax_consistency_constraint]))
                            >= x[lanelet_mapping[lt_id], k + 1 + self.relax_consistency_constraint])

        m.optimize()

        if m.Status != 2:
            raise NotImplementedError  # Solution status not optimal. Maybe try higher value for relax_consistency
